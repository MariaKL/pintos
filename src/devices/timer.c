...
/* List of all processes in THREAD_BLOCKED state - processes 
...
timer_sleep(int64_t ticks) {
    ...
    //printf("Wake me up at %" PRIu64 " ticks.\n",(uint64_t)thread_current()->endTime);
...
timer_interrupt(struct intr_frame *args UNUSED) {
    ...
    /*t = (list_entry (list_front(&blocked_list), struct thread, belem));
        if(timer_ticks()< t->endTime){ 
            // nothing
        }
        else{
